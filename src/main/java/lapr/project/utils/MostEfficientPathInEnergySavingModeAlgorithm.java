...
public class MostEfficientPathInEnergySavingModeAlgorithm implements Algorithm{
     ...
     * @param vehicle
     * @param gear
     * @return the gear below the current one (or the same in case that is already the lowest)
     */
    ...
    public Gear selectGear(Vehicle vehicle, double vr, double angle){
        ...
        int tq = regime.getTorqueLow(); 
            while(getForceAppliedToVehicle(tq, vehicle.getEnergy().getFinalDriveRatio(), gear.getRatio(), vehicle.getWheelSize(), vehicle.getRrc(), mass, vehicle.getDrag(), vehicle.getFrontalArea(), vr, angle) != 0 || tq < regime.getTorqueHigh()){
